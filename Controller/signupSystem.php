...
class signupSystem
{
    ...
    public function signup($username, $email, $password, $passwordRepeat, $package)
    {
        ...
        $check = $this->createUser($data, $username, $email, $hashedPwd, $package);
    ...
    private function createUser($data, $username, $email, $hashedPwd, $package)
    {
        $hashedPwd = password_hash($password, PASSWORD_DEFAULT);
        loggerSingleton::getInstance()->writeLog("Pwd hash ", levelLogger::DEBUG);
        $data = dbSingleton::getInstance()->execSQL("INSERT INTO user(uid_user, email_user, password_user, fk_package_id, fk_role_id) VALUES(?,?,?,?,1);", array($username, $email, $hashedPwd, $package));
